...
const SB_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNoZ2VnZmNuZnppaG15dGdiaGpoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzg4NzQ1NTQsImV4cCI6MjA5NDQ1MDU1NH0.mIedSmjRtZGl1nnQR7h_mfl7HX7BLz67mGdF17uzXkM";
const ADMIN_KEY = "khelakokhon2026admin";

/* ── Supabase ────────────────────────────────── */
async function sb(path, opts={}) {
  const r = await fetch(SB_URL+"/rest/v1/"+path, {
    headers:{"apikey":SB_KEY,"Authorization":"Bearer "+SB_KEY,"Content-Type":"application/json",...(opts.h||{})},
    method:opts.method||"GET", body:opts.body,
  });
  const t=await r.text(); return t?JSON.parse(t):[];
}
const getScores    = ()=>sb("scores?select=*");
const getPreds     = n=>sb("predictions?predictor_name=eq."+encodeURIComponent(n)+"&select=*");
const getLB        = ()=>sb("predictions?select=predictor_name,points");
const checkDevice  = did=>sb("predictors?device_id=eq."+encodeURIComponent(did)+"&select=name");
const checkName    = n=>sb("predictors?name=eq."+encodeURIComponent(n)+"&select=device_id");

async function getDeviceId(){
  let id=localStorage.getItem("kk_did");
  if(!id){
    ...
    id="d_"+Math.abs(h).toString(36)+"_"+Date.now().toString(36);
    localStorage.setItem("kk_did",id);
  }
  return id;
}
async function registerUser(name,did){
  const nc=await checkName(name);
  if(nc.length>0&&nc[0].device_id!==did) throw new Error("name_taken");
  await sb("predictors",{method:"POST",h:{"Prefer":"resolution=merge-duplicates"},body:JSON.stringify({name,device_id:did})});
}
async function upsertPred(name,matchId,hg,ag){
  return sb("predictions",{method:"POST",h:{"Prefer":"resolution=merge-duplicates,return=representation"},
    body:JSON.stringify({predictor_name:name,match_id:matchId,home_score:hg,away_score:ag,points:0})});
}
async function deletePred(name,matchId){
  return sb("predictions?predictor_name=eq."+encodeURIComponent(name)+"&match_id=eq."+matchId,{method:"DELETE"});
}
async function saveScoreDB(matchId,hg,ag){
  await sb("scores",{method:"POST",h:{"Prefer":"resolution=merge-duplicates"},
    body:JSON.stringify({match_id:matchId,home_score:hg,away_score:ag})});
  const preds=await sb("predictions?match_id=eq."+matchId+"&select=*");
  for(const p of preds){
    const ph=p.home_score,pa=p.away_score;
    let pts=0;
    if(ph===hg&&pa===ag) pts=3;
    else{const ar=hg>ag?"h":hg<ag?"a":"d",pr=ph>pa?"h":ph<pa?"a":"d";if(ar===pr)pts=1;}
    await sb("predictions?id=eq."+p.id,{method:"PATCH",h:{"Prefer":"return=representation"},body:JSON.stringify({points:pts})});
  }
}
