...
const GAS_URL = "https://script.google.com/macros/s/AKfycbyhb7tV0bDJFfHL7Ih1_pEkaAdIrBoii1eZZB76_4QAzce_yOCDphAegKZCT0KYBAI/exec";
const SECRET = "khelakokhon2026";
const ADMIN_KEY = "khelakokhon2026admin";
const FD_KEY = "b3a4370cd7434e769f385e0f48aaf843";
const FD_BASE = "https://api.football-data.org/v4";

// Popular clubs with football-data.org IDs
const CLUBS = [
  {id:57,name:"আর্সেনাল",en:"Arsenal",crest:"https://crests.football-data.org/57.png",league:"PL"},
  {id:61,name:"চেলসি",en:"Chelsea",crest:"https://crests.football-data.org/61.png",league:"PL"},
  {id:64,name:"লিভারপুল",en:"Liverpool",crest:"https://crests.football-data.org/64.png",league:"PL"},
  {id:65,name:"ম্যান সিটি",en:"Man City",crest:"https://crests.football-data.org/65.png",league:"PL"},
  {id:66,name:"ম্যান ইউ",en:"Man Utd",crest:"https://crests.football-data.org/66.png",league:"PL"},
  {id:73,name:"টটেনহ্যাম",en:"Spurs",crest:"https://crests.football-data.org/73.png",league:"PL"},
  {id:86,name:"রিয়াল মাদ্রিদ",en:"Real Madrid",crest:"https://crests.football-data.org/86.png",league:"PD"},
  {id:81,name:"বার্সেলোনা",en:"Barcelona",crest:"https://crests.football-data.org/81.png",league:"PD"},
  {id:5,name:"বায়ার্ন",en:"Bayern",crest:"https://crests.football-data.org/5.png",league:"BL1"},
  {id:524,name:"পিএসজি",en:"PSG",crest:"https://crests.football-data.org/524.png",league:"FL1"},
  {id:98,name:"এসি মিলান",en:"AC Milan",crest:"https://crests.football-data.org/98.png",league:"SA"},
  {id:109,name:"জুভেন্টাস",en:"Juventus",crest:"https://crests.football-data.org/109.png",league:"SA"},
  {id:99,name:"ইন্টার মিলান",en:"Inter",crest:"https://crests.football-data.org/99.png",league:"SA"},
  {id:76,name:"আটলেটিকো",en:"Atletico",crest:"https://crests.football-data.org/78.png",league:"PD"},
  {id:4,name:"ডর্টমুন্ড",en:"Dortmund",crest:"https://crests.football-data.org/4.png",league:"BL1"},
];

function toBDT(utcDate,lang){
  const d=new Date(utcDate);
  if(lang==="bn") return new Intl.DateTimeFormat("bn-BD",{timeZone:"Asia/Dhaka",day:"numeric",month:"long",hour:"numeric",minute:"2-digit",hour12:true}).format(d);
  return new Intl.DateTimeFormat("en-GB",{timeZone:"Asia/Dhaka",day:"numeric",month:"short",hour:"numeric",minute:"2-digit",hour12:true}).format(d);
}

function getCountdown(utcDate,lang){
...
function ClubFootballTab({T,lang}){
  const[club,setClub]=useState(null);
  const[matches,setMatches]=useState([]);
  const[loading,setLoading]=useState(false);
  const[search,setSearch]=useState("");

  const fetchMatches=async(c)=>{
    setClub(c);setLoading(true);setMatches([]);
    try{
      const r=await fetch(FD_BASE+"/teams/"+c.id+"/matches?status=SCHEDULED&limit=5",{headers:{"X-Auth-Token":FD_KEY}});
  ...
  const filtered=search?CLUBS.filter(c=>c.en.toLowerCase().includes(search.toLowerCase())||c.name.includes(search)):CLUBS;

  if(club) return(
    <div style={{padding:16,paddingBottom:80}}>
      <button onClick={()=>{setClub(null);setMatches([]);}} style={{display:"flex",alignItems:"center",gap:8,background:T.card2,border:`1px solid ${T.border}`,color:T.text,borderRadius:8,padding:"8px 14px",fontFamily:HS,fontSize:13,cursor:"pointer",marginBottom:16}}>
        ← {lang==="bn"?"ফিরে যান":"Back"}
      </button>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,background:T.card,borderRadius:12,padding:14,border:`1px solid ${T.border}`}}>
        <img src={club.crest} style={{width:44,height:44,objectFit:"contain"}} alt="" onError={e=>e.target.style.display="none"}/>
        <div style={{fontFamily:HS,fontSize:18,fontWeight:800,color:T.text}}>{lang==="bn"?club.name:club.en}</div>
      </div>
      {loading?<div style={{textAlign:"center",padding:40,fontFamily:HS,color:T.textS}}>{lang==="bn"?"লোড হচ্ছে...":"Loading..."}</div>
      :matches.length===0?<div style={{textAlign:"center",padding:40,fontFamily:HS,color:T.textS,background:T.card,borderRadius:12,border:`1px solid ${T.border}`}}>{lang==="bn"?"কোনো upcoming match নেই":"No upcoming matches"}</div>
      :matches.map(m=>{
        const isLive=m.status==="IN_PLAY"||m.status==="PAUSED";
        const isFT=m.status==="FINISHED";
        const cd=!isFT&&!isLive?getCountdown(m.utcDate,lang):null;
        return(
          <div key={m.id} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:14,padding:16,marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{fontFamily:HS,fontSize:11,color:T.textM}}>{m.competition?.name}</div>
              {isLive&&<div style={{background:T.red,color:"#fff",fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:20,animation:"pulse 1s infinite"}}>LIVE</div>}
              {isFT&&<div style={{fontFamily:HS,fontSize:11,color:T.textS}}>FT</div>}
            </div>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <div style={{flex:1,display:"flex",alignItems:"center",gap:8,minWidth:0}}>
                <img src={m.homeTeam.crest} style={{width:28,height:28,objectFit:"contain",flexShrink:0}} alt="" onError={e=>e.target.style.display="none"}/>
                <div style={{fontFamily:HS,fontSize:14,fontWeight:600,color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{m.homeTeam.shortName||m.homeTeam.name}</div>
              </div>
              <div style={{textAlign:"center",minWidth:60,flexShrink:0}}>
                {isFT||isLive
                  ?<div style={{fontFamily:HS,fontSize:20,fontWeight:800,color:T.text}}>{m.score?.fullTime?.home??0} - {m.score?.fullTime?.away??0}</div>
                  :<div style={{fontFamily:HS,fontSize:12,color:T.green,fontWeight:700}}>vs</div>}
              </div>
              <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"flex-end",gap:8,minWidth:0}}>
                <div style={{fontFamily:HS,fontSize:14,fontWeight:600,color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",textAlign:"right"}}>{m.awayTeam.shortName||m.awayTeam.name}</div>
                <img src={m.awayTeam.crest} style={{width:28,height:28,objectFit:"contain",flexShrink:0}} alt="" onError={e=>e.target.style.display="none"}/>
              </div>
            </div>
            {!isFT&&(
              <div style={{marginTop:12,textAlign:"center",borderTop:`1px solid ${T.border}`,paddingTop:10}}>
                <div style={{fontFamily:HS,fontSize:12,color:T.textS}}>{toBDT(m.utcDate,lang)}</div>
                {cd&&<div style={{fontFamily:HS,fontSize:11,color:T.green,marginTop:4}}>⏱️ {cd}</div>}
              </div>
            )}
          </div>
        );
  ...
  return(
    <div style={{padding:16,paddingBottom:80}}>
      <input value={search} onChange={e=>setSearch(e.target.value)}
        placeholder={lang==="bn"?"🔍 দল খুঁজুন...":"🔍 Search team..."}
        style={{width:"100%",padding:"10px 14px",borderRadius:10,border:`1px solid ${T.border}`,background:T.card2,color:T.text,fontFamily:HS,fontSize:13,boxSizing:"border-box",marginBottom:16,outline:"none"}}/>
      <div style={{fontFamily:HS,fontSize:12,fontWeight:700,color:T.textS,marginBottom:12}}>{lang==="bn"?"জনপ্রিয় দল":"Popular Teams"}</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
        {filtered.map(c=>(
          <div key={c.id} onClick={()=>fetchMatches(c)} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:14,cursor:"pointer",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
            <img src={c.crest} style={{width:44,height:44,objectFit:"contain"}} alt="" onError={e=>e.target.style.display="none"}/>
            <div style={{fontFamily:HS,fontSize:12,fontWeight:600,color:T.text}}>{lang==="bn"?c.name:c.en}</div>
          </div>
        ))}
      </div>
    </div>
...
async function gasPost(action, data={}) {
  const payload = {action, secret: SECRET, ...data};
  const params = "data=" + encodeURIComponent(JSON.stringify(payload));
  try {
    ...
    try { fetch(GAS_URL + "?" + params, {mode:"no-cors"}); } catch(e2) {}
    return {ok: true};
  }
}
...
const SPONSOR_AD = {
  ...
  subBn: "আর্জেন্টিনা ফ্যান বনাম ব্রাজিল ফ্যান",
  subEn: "Argentina Fan vs Brazil Fan",
  ctaBn: "লাইভ দেখুন, গোল দিন →",
  ctaEn: "Watch live, score a goal →",
};
  ...
  return(
    <>
      ...
      <div style={{background:T.bg,minHeight:"100vh",maxWidth:480,margin:"0 auto",fontFamily:HS,transition:"background .3s"}}>
        ...
        <div style={{background:T.hdr,position:"sticky",top:0,zIndex:50,boxShadow:"0 2px 20px rgba(0,0,0,0.4)"}}>
          ...
          <div style={{display:"flex",borderTop:"1px solid rgba(255,255,255,0.1)"}}>
            {[
              ...
              {id:"club",label:lang==="bn"?"ক্লাব":"Club",icon:<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 2a14.5 14.5 0 000 20 14.5 14.5 0 000-20"/><path d="M2 12h20"/></svg>},
            ].map(({id,label,icon})=>(
        ...
        {mt==="club"&&<ClubFootballTab T={T} lang={lang}/>}
